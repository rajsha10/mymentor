...
export default function ClassroomView() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-sans text-black selection:bg-brand-coral/30">
      ...
      {/* ── Top Navigation Bar ── */}
      <nav className="bg-white border-b-2 border-black sticky top-0 z-30 py-3">
          ...
          <div className="flex justify-between items-center h-14">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="lg:hidden flex items-center justify-center w-10 h-10 border-2 border-black rounded-xl hover:bg-black hover:text-white transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none"
              ...
              <button
                onClick={() => navigate(-1)}
                className="group flex items-center justify-center w-10 h-10 border-2 border-black rounded-xl hover:bg-black hover:text-white transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none"
                title="Go Back"
              >
                <span className="text-xl font-bold group-hover:-translate-x-0.5 transition-transform">&larr;</span>
              </button>

              <div className="flex flex-col">
                <div className="flex items-center gap-3">
                  <h1 className="text-xl sm:text-2xl font-black tracking-tight text-black">{classroom.name}</h1>
                  {classroom.meetingActive && (
                    <div className="flex items-center px-3 py-1 bg-brand-coral text-black border-2 border-black rounded-lg text-[10px] font-black uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] animate-pulse">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-0.5 border border-gray-200 rounded-md">
                    Class ID: {classroom.classroomId}
                  </span>
                  {isOwner && (
                    <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-0.5 border border-blue-200 rounded-md">
                      Teacher View
                    </span>
                  )}
                </div>
            ...
            <div className="flex items-center gap-4">
              <img src="/logo.png" alt="MyMentor" className="h-10 w-auto object-contain hidden md:block" />
              <div className="h-8 w-[2px] bg-black/10 hidden md:block mx-2" />
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex flex-col items-end">
                  <span className="text-xs font-black text-black">{userData?.name || 'Teacher'}</span>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">{userData?.role || 'Educator'}</span>
                </div>
                <div className="w-10 h-10 border-2 border-black rounded-xl bg-brand-coral/10 flex items-center justify-center font-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  {userData?.name?.[0] || 'T'}
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-1 relative">
        ...
        <>
            ...
            <aside
              className={`
                fixed top-[82px] left-0 h-[calc(100vh-82px)] w-60
            ...
            >
              {/* Mobile header */}
              <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b-2 border-black bg-gray-50">
                <span className="font-black text-xs uppercase tracking-widest text-gray-500">Navigation</span>
                <button
                  onClick={() => setSidebarOpen(false)}
                  className="flex items-center justify-center w-8 h-8 border-2 border-black rounded-lg hover:bg-black hover:text-white transition-all"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>

              {/* Classroom label in sidebar */}
              <div className="px-4 py-4 border-b-2 border-black">
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Classroom</p>
                <p className="text-sm font-black text-black leading-tight line-clamp-2">{classroom.name}</p>
              </div>
              ...
              <nav className="flex flex-col gap-1 p-3">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`
                      flex items-center gap-3 w-full px-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider text-left transition-all
                      ${activeTab === tab.id
                        ? 'bg-black text-white shadow-[4px_4px_0px_0px_rgba(255,107,87,1)] translate-x-1'
                        : 'text-black hover:bg-gray-100 hover:translate-x-1 border-2 border-transparent hover:border-black/20'
                      }
                    `}
                  >
                    {TAB_ICONS[tab.id]}
                    ...
                    {tab.id === 'participants' && classroom.pendingRequests?.length > 0 && (
                      <span className="bg-brand-coral text-black border border-black py-0.5 px-1.5 rounded-md text-[9px] shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] animate-bounce">
        ...
        {/* Main content area */}
        <main className="flex-1 min-w-0 py-8 px-4 sm:px-6 lg:px-8 lg:ml-60">
          <div className="bg-white min-h-[70vh] rounded-[2.5rem] border-2 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col">
