...
function QuestionCard({ q, index }: { q: TestQuestion; index: number }) {
  return (
    <div className="group border-2 border-black rounded-3xl p-6 bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all">
      <div className="flex flex-wrap items-center gap-2.5 mb-4">
        <span className="text-[9px] font-black uppercase bg-black text-white px-2.5 py-1 rounded-lg shrink-0 shadow-[1.5px_1.5px_0px_0px_rgba(255,107,87,1)]">
          QUESTION {index + 1}
        </span>
        <span className="text-[9px] font-black uppercase border-2 border-black bg-gray-50 px-2.5 py-1 rounded-lg text-black shrink-0">
          {QUESTION_TYPE_LABELS[q.type] ?? q.type}
        </span>
        <div className="ml-auto h-0.5 flex-1 bg-black/5 rounded-full" />
      </div>
      
      <p className="text-base font-black text-black mb-4 leading-relaxed uppercase tracking-tight">{q.question}</p>
        ...
        <div className="grid grid-cols-1 gap-2.5 mb-4">
          {q.options.map((opt, i) => {
            ...
            return (
              <div
                key={i}
                className={`text-xs px-4 py-3 rounded-xl border-2 font-black transition-all ${
                  isCorrect
                    ? 'bg-green-50 border-black text-green-700 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
      ...
      {!q.options && (
        <div className="bg-gray-50 border-2 border-black border-dashed rounded-xl px-5 py-4">
          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Pedagogical Solution</p>
          <p className="text-xs font-bold text-black leading-relaxed">{q.answer}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="p-6 sm:p-8 max-w-5xl mx-auto space-y-10 bg-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 border-b-4 border-black pb-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-2xl font-black text-black leading-none tracking-tight">Assessment Engine</h2>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-[#FDFDFD] border-4 border-black rounded-3xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-6 space-y-6">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-[10px] font-black text-black uppercase tracking-[0.3em]">Generation Parameters</h3>
            </div>
            ...
            {/* Difficulty */}
            <div className="space-y-2">
              <label className="block text-[9px] font-black text-gray-400 uppercase tracking-[0.3em]">Complexity Level</label>
              <div className="grid grid-cols-3 gap-2">
                {DIFFICULTIES.map(d => (
                  <button
                    key={d}
                    onClick={() => setDifficulty(d)}
                    className={`py-2 rounded-lg border-2 font-black text-[9px] uppercase tracking-widest transition-all ${
                      difficulty === d
                        ? 'bg-black text-white border-black shadow-[2px_2px_0px_0px_rgba(255,107,87,1)] -translate-y-0.5'
            ...
            {/* Question Types */}
            <div className="space-y-2">
              <label className="block text-[9px] font-black text-gray-400 uppercase tracking-[0.3em]">Assessment Formats</label>
                ...
                {(Object.keys(QUESTION_TYPE_LABELS) as QuestionType[]).map(t => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-lg border-2 font-black text-[9px] uppercase tracking-widest text-left transition-all ${
                        selected
            ...
            >
              {loading ? (
                ...
                <><Bot className="h-4 w-4" /> GENERATE ASSESSMENT</>
              )}
            </button>
          </div>
        </div>
          ...
          {loading && (
            <div className="h-full min-h-[400px] flex flex-col items-center justify-center gap-6 border-4 border-black border-dashed rounded-3xl bg-gray-50/50 p-8">
              ...
              <div className="space-y-1 text-center">
                ...
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.4em] animate-pulse">Scanning knowledge repository…</p>
              </div>
            </div>
          )}
              ...
              <div className="flex items-center justify-between bg-black rounded-3xl p-6 text-white shadow-[6px_6px_0px_0px_rgba(255,107,87,1)]">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded-lg border border-white/20 bg-white/5">
                      {result.difficulty} COMPLEXITY
                    </span>
                    {result.question_types.map(t => (
                      <span key={t} className="text-[8px] font-black uppercase px-2 py-0.5 rounded-lg border border-white/20 bg-brand-coral/10 text-brand-coral">
                        {QUESTION_TYPE_LABELS[t]}
                      </span>
                    ))}
                  </div>
                ...
                {saved && (
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest text-green-400">Archived</span>
                  </div>
                )}
              </div>
